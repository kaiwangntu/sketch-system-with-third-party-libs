...
public:
	...
	int PaintingOnBFPlane(Plane_3 BestFittingPlane,GLdouble* modelview,GLdouble* projection,GLint* viewport,
		const vector<CPoint> DeformCurvePoint,vector<Point_3>& DeformCurvePoint3d);
	...
	bool PaintingScrPointOnFrontalMesh(KW_Mesh& Mesh,CPoint UserScrPoint,Point_3& UserPoint,Facet_handle& UserPointFacet,
		GLdouble* modelview,GLdouble* projection,GLint* viewport);

	//circle to select the vertices on the front of the mesh
	int CircleFrontVertices(KW_Mesh& Mesh,vector<CPoint> vecBoundingCurve,
		GLdouble* modelview,GLdouble* projection,GLint* viewport,
		vector<Vertex_handle>& vecCirVer);

	//circle to select the vertices on both the front and back of the mesh
	int CircleAllVertices(KW_Mesh& Mesh,vector<CPoint> vecBoundingCurve,
		GLdouble* modelview,GLdouble* projection,GLint* viewport,
		vector<Vertex_handle>& vecCirVer);
