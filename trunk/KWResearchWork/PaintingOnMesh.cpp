...
int CPaintingOnMesh::CircleFrontVertices(KW_Mesh& Mesh,vector<CPoint> vecBoundingCurve, 
										   ...
										   vector<Vertex_handle>& vecCirVer)
{
	...
	DBWindowWrite("selected ROI Num: %d\n",vecTempROI.size());
...
int CPaintingOnMesh::CircleAllVertices(KW_Mesh& Mesh,vector<CPoint> vecBoundingCurve, GLdouble* modelview,GLdouble* projection,GLint* viewport,
									   vector<Vertex_handle>& vecCirVer)
{
	//compute the 2D bounding polygon first
	Polygon_2 BoundingPolygon;
													 ...
													 vector<Vertex_handle>& vecHandleNbVertex)
{
	...
	{
		AfxMessageBox("Invalied Curve! Draw again please");
		return 0;
	}
...
int CPaintingOnMesh::PaintingPointOnSphere(Sphere_3 sphere,GLdouble* modelview,GLdouble* projection,GLint* viewport, 
									  CPoint InputPoint,vector<Point_3>& IntersectPoints)
{
	//first convert UCP into opengl coordinate(on Znear) 
	HPCPoint pt;
	pt.x =InputPoint.x;
...
bool CPaintingOnMesh::PaintingScrPointOnFrontalMesh(KW_Mesh& Mesh,CPoint UserScrPoint,Point_3& UserPoint,Facet_handle& UserPointFacet, 
													GLdouble* modelview,GLdouble* projection,GLint* viewport)
{
