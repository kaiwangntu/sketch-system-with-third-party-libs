...
bool CCrossSectionProc::CheckIntersectOthers(CurveNetwork CnToCheck,Polygon_2 NewProfile2D,int iExclProfileInd/*=-1*/)
{
	...
	for (unsigned int i=0;i<CnToCheck.Profile2D.size();i++)
	{
		...
		if (iResult==2)
		{
			DBWindowWrite("Error: The input cross section intersects with other cross sections...\n");
			return true;
		}
	}
	return false;
...
void CCrossSectionProc::SetCNOrientation(CurveNetwork& CnToSet)
{
	...
	for (unsigned int iCurve=0;iCurve<CnToSet.Profile2D.size();iCurve++)
	{
		...
		DBWindowWrite("in tree, %d th curve\n",iCurve);
		...
		while (true)
		{
			...
			else
			{
				...
				DBWindowWrite("compare with %d th curve\n",*sibIter);
									   ...
									   vector<Point_3>& vecCurvePlaneIntersectPoint,vector<int>& vecCurvePlaneIntersectType)
{
	...
	for (unsigned int iPlane=0;iPlane<vecCurveNetwork.size();iPlane++)
	{
		...
		if (iIntersectNumAB!=iIntersectNumBA)
		{
			...
			DBWindowWrite("rule 4 is violated...,Existing CN id: %d,Specified CN id: %d,\n",iPlane,iSpecifiedCN);
			bResult=false;
			break;
		}
...
bool CCrossSectionProc::ImportCrossSections(char* fnames,vector<CurveNetwork>& vecCurveNetwork)
{
	...
	if( fin == NULL )
	{
		DBWindowWrite("Unable to open file\n");
		return false;
	}
	...
	for( int i = 0; i < ctrnum; i ++)
	{
		...
		{
			DBWindowWrite("error: unknown plane type!\n");
		}
	...
	if( pPartFile == NULL )
	{
		DBWindowWrite("Unable to open part file\n");
		return true;
	}
	int iPartialCN=0;
