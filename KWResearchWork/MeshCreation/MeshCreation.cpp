...
void CMeshCreation::SetDrawingPlane()
{
	...
	else if ((iSelectedName>=CREATION_SKETCH_CURVE_NAME_BEGIN)&&(iSelectedName<=CREATION_SKETCH_CURVE_NAME_END))
	{
		...
		DBWindowWrite("%dth CN,%dth CS\n",this->iSelectedCN,this->iSelectedCS);
...
bool CMeshCreation::CheckPlaneState()
{
	...
	{
		AfxMessageBox("Have u selected a plane?");
	...
	if (this->bAutoRot)
	{
		...
		{
			AfxMessageBox("Make the selected plane face to u pls");
			return CREATION_PLANE_NOT_READY_FOR_DRAWING;
		}
	}
...
void CMeshCreation::Input2DProfilePoint(CPoint ProfilePoint)
{
...
//	if (NewProfile.size()==SAMPLE_POINT_NUM)//20
	if (!NewProfile.empty())
	{
		...
		{
			DBWindowWrite("new cs self-intersected\n");
			AfxMessageBox("Invalid Input,violate Rule 3!");
		...
		if (!this->vecCurveNetwork.empty())//not first curve
		{
			...
			if (iCurveType==1)
			{
				...
				{
					DBWindowWrite("modified cs intersected with other cs,curve type:%d\n",iCurveType);
					AfxMessageBox("Invalid Input,violate Rule 3!");
					this->UserInput2DProfile.clear();
					return;
				}
...
void CMeshCreation::GenerateMesh(KW_Mesh& Mesh,vector<double> vecMeshColor)
{
	...
	////test
	//ImplicitMesher ImpMesher;
	//ImpMesher.ContourToMesh(this->vecCurveNetwork,Mesh,this->vecTestPoint);
