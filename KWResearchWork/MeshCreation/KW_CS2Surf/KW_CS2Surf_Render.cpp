...
void KW_CS2Surf::RenderSubspaceStl()
{
	...
	for (int i=0;i<this->iSSspacenum;i++)
	{
		...
		//for (int j=0;j<this->vecRenderSSspacefacenum.at(i);j++)
		{
			int iFaceID=this->vecvecSSspace.at(i).at(j);
			//int iFaceID=this->vecvecRenderSSspace.at(i).at(j);
			ResortedFace FaceInfo=this->vecResortFace.at(iFaceID);
			//ResortedFace FaceInfo=this->vecRenderResortFace.at(iFaceID);
...
void KW_CS2Surf::RenderPolyh()
{
	if (this->iRenderSinglePoly==CR_RENDER_NONE_CYLINDER)
	{
		return;
	}
